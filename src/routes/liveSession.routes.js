...
function sanitizeZoomInput(input = {}) {
  ...
  if (url && isHttps(url) && allowedHost(url)) out.joinUrl = url;
  if (startUrl && isHttps(startUrl) && allowedHost(startUrl)) out.startUrl = startUrl;
  if (pass) out.passcode = pass.slice(0, 64);
  return Object.keys(out).length ? out : undefined;
}
...
function isMemberActive(mem) {
  ...
  return (
    ...
    now >= mem.currentPeriodStart &&
    now < mem.currentPeriodEnd
  );
}

function shortToken(len = 12) {
  return crypto.randomBytes(len).toString('base64url');
}
function makeDummyJoinUrl(sessionId) {
  return `${FRONTEND_URL}/live/${sessionId}/join?d=${shortToken()}`;
}
...
// small helper to normalize thumbnail input (base64 or URL)
async function processThumbnailField(thumbnailRaw) {
  if (!thumbnailRaw) return undefined;
  if (thumbnailRaw.startsWith && thumbnailRaw.startsWith('data:image/')) {
    // base64 -> buffer -> upload
    const base64Data = thumbnailRaw.split(',')[1];
    const buffer = Buffer.from(base64Data, 'base64');
    const uploaded = await uploadBufferToCloudinary(buffer, {
      folder: 'live-thumbnails',
    });
  ...
  // if it's already an http(s) URL, trust it
  if (/^https?:\/\//i.test(thumbnailRaw)) {
    return thumbnailRaw;
  }
  // anything else => ignore instead of exploding
  return undefined;
}
...
router.post('/', authGuard, requireRole('instructor', 'admin'), async (req, res) => {
  try {
    ...
    // process thumbnail (base64 -> cloudinary)
    let thumbnailUrl;
    try {
      thumbnailUrl = await processThumbnailField(thumbnail);
    } catch (err) {
      console.error("Cloudinary upload failed:", err);
      return res.status(500).json({ error: 'Thumbnail upload failed' });
    }
...
router.patch('/:id', authGuard, requireRole('instructor', 'admin'), async (req, res) => {
  try {
    ...
    if (req.body.membersAccess) {
      updates.membersAccess = req.body.membersAccess;
    }

    // thumbnail: allow change/removal + handle base64
    if (typeof req.body.thumbnail === 'string') {
      if (req.body.thumbnail.trim() === '') {
        updates.thumbnail = undefined;
      } else {
        try {
          updates.thumbnail = await processThumbnailField(req.body.thumbnail);
        } catch (err) {
          console.error("Cloudinary upload failed (PATCH):", err);
          return res.status(500).json({ error: 'Thumbnail upload failed' });
        }
      }
    }
    ...
    if (req.body.zoom) {
      const z = sanitizeZoomInput(req.body.zoom);
      if (z) updates.zoom = z;
      else updates.zoom = undefined; // clear
    }
...
/* DELETE */
router.delete('/:id', authGuard, requireRole('instructor', 'admin'), async (req, res) => {
  try {
    const uid = getUserId(req);
    const s = await LiveSession.findById(req.params.id);
    if (!s) return res.status(404).json({ error: 'Not found' });
    ...
    await LiveSession.deleteOne({ _id: s._id });
    return res.json({ ok: true });
  } catch (err) {
    console.error('DELETE /live-sessions/:id error:', err?.message);
