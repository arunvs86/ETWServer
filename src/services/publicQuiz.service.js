...
function shapeQuestionForPlay(q) {
  return {
    id: String(q._id),                 // ← cast
    type: q.type,
    prompt: q.prompt,
    media: Array.isArray(q.media) ? q.media : [],
    options: (q.options || []).map(o => ({
      id: String(o.id || ''),          // ← cast (your IDs are A/B etc; keep as string)
      text: o.text,
...
function shapeQuestionForReview(q) {
  return {
    ...
    points: q.points,
    explanation: q.explanation || '',
    correctOptionIds: (q.correctOptionIds || []).map(String), // ← cast too
    correctBoolean: typeof q.correctBoolean === 'boolean' ? q.correctBoolean : undefined,
    correctText: (q.correctText || []).map(String),
  };
}
